...
- **VITAL** — a birth/death date or place differs across platforms. *Signature:* same person, different date or place. The dangerous sub-cases are anachronisms (a place that did not exist at the stated date), transatlantic-impossibles, and a date contradicted by the person's own children — and the most important thing to internalise is that a large VITAL gap is frequently an IDENTITY or CONFLATION problem wearing a date's clothing: a twenty-year death-date disagreement on a "confirmed" person usually means the linked profile is a different same-name individual, or two people are fused, not that one date is mistyped. The most common *benign* sub-case is the opposite, a stale snapshot — the observed claim was captured months ago and the platform has since corrected its own value, so a live re-fetch shows the platforms now agree and the flag was never a real conflict. These two poles, identity-in-disguise and stale-snapshot, are why a VITAL flag is worth a cheap look before it is either trusted or dismissed.
...
A reconciliation architecture is only useful if you can work the backlog it produces down to something small, and at the scale automated producers generate — thousands of open disagreements across several trees — you cannot research each one by hand, nor should you, because most of them are not real. The discipline that makes the pile tractable is a two-tier split: classify the mechanical same-person artifacts and close them by rule, and reserve human-gated research for the genuine conflicts. The mechanical tier is large and safe to batch because the divergences are not disagreements about *who* the person is, only about how a value is written — a place given at two granularities ("Banff, Banffshire, Scotland" against "Scotland"), the same town assigned to two counties because a county was split from its parent between the two records, a spelling or transcription variant ("Northamptonshire" against "Northhamptonshire"), a birth-year estimate that differs by a few years, or a platform's relatives-scrape that packed both parents into one field so a single-parent comparison read as a mismatch. Each of those closes as *disproven* on a reasonable assumption, journal-only and reversible. One caveat draws the tier's boundary and it is the whole game: the assumption holds only where the identity itself is not in question, which in practice means a confirmed cross-platform id and no open conflation, and it must never be extended to the graft-prone calls, because a name-string heuristic cannot tell a competing same-role parent from a legitimate co-parent, and the moment it tries you have auto-closed a real problem.

What is left after the mechanical tier is the genuine conflicts, and here the non-negotiable rule is that no conflict is closed on a single pass. Every research verdict goes to an independent adversarial verifier whose job is to *refute* it: to re-fetch the platform profile and, crucially, an independent down-line child rather than trusting the first analyst's child claim, and to default to *held* on any doubt. This second pass earns its cost repeatedly, because it is the step that catches a first analyst who declared "same person, spurious flag" on a node whose linked id actually resolves to a wrong-century individual, or whose date gap is the fingerprint of two fused people, and flips the verdict from disproven back to held. The rule that falls out of it is blunt: never mechanically downgrade a live conflict without an independent down-line re-fetch, since the wrong-id-to-a-same-name-person case is invisible to any research that trusts the stored mapping.

Not all genuine conflicts deserve the same effort, and the prioritisation that pays is shallow-and-recent first. Conflicts in the well-documented colonial and modern layers resolve into real, actionable findings at a high rate, a wrong parent to re-point or two people to split or a wrong id to detach, whereas the deep medieval-nobility tail is mostly fabricated-graft and name-variant noise where the tree and the platform inherited the same bad pedigree and no surviving record will adjudicate a twelve-year spread on a person born a thousand years ago. Working the tractable layer first turns the same research budget into far more corrected conclusions. It is worth noticing that a shallow VITAL date conflict, followed rather than dismissed, is one of the cheapest identity-error detectors available: chasing "which death year is right" on recent, well-sourced people repeatedly surfaces two-person fusions and wrong-id mappings that no producer had flagged, because the date gap was the only visible symptom of an underlying identity problem.

The last thing to keep in view is which output is the product. Draining the backlog yields two piles, and the closed-as-disproven pile is cleared noise rather than the goal; the value is the *held* pile, because a held verdict is a curated worklist entry rather than a dead end — it carries the recommended action (re-point this parent, detach that id, correct this date to the one the record supports) and the specific next record to pull, so a human resolves it in minutes instead of re-deriving the whole tangle. A reconciliation program that closes ten thousand flags and produces no held worklist has mostly demonstrated that its trees already agreed, whereas the one that produces a few hundred well-evidenced held recommendations, each with its next record, is where the corrections actually come from.
