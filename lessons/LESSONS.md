...
**Rule [CONFIRMED ×3] (captured 2026-04-15): Trees are leads, never sources.** `POST /platform/tree/relationships` with type "ParentChild" creates couple relationships on FamilySearch regardless — the correct endpoint is `POST /platform/tree/child-and-parents-relationships`. More broadly, every tree is a lead. **Amended 2026-08-08**: the curated shared trees (FamilySearch Family Tree, WikiTree, Geni) are Tier 3 and the private member trees (Ancestry, MyHeritage, Rootsweb/WorldConnect) are Tier 5 — see `methodology/02-evidence-standards.md` §Source Tier Hierarchy for the reasoning. The tier differs because a shared tree carries an edit history you can audit and argue with; the handling does not differ at all. They point to records; they are not themselves evidence. Extract the attached records and cite those. A parent link built on tree-only evidence is POSSIBLE confidence at best and should carry an `upgrade_path` field describing what T1-3 evidence would promote it.
...
**Rule [CONFIRMED ×2] (captured 2026-04-15, formalized 2026-08-08): Obituaries name relationships explicitly, and the tier depends on where it was published.** Full obituary text is strong evidence for parents, siblings, and children — often superior to vital record indexes when those records are unavailable. An obituary in a verifiable newspaper archive (Newspapers.com, Chronicling America, digitized local papers) is **Tier 2b**; a funeral-home or memorial-site obituary with no newspaper attribution is family-written with no editorial review and is **Tier 4**. When in doubt, establish whether the text was published in a newspaper or only on a memorial site. Search newspaper archives before deferring to onsite research. *(kindred, dry-cross)*
...
These lessons come from draining the disagreement backlog (Chapter 7 architecture) down across three trees: the PARENTAGE-conflict drain to zero and the VITAL-tier batch-classify + shallow-research pass in 2026-07. See `methodology/07-cross-platform-reconciliation.md` "Draining the backlog" for the connected-prose version.

**Rule [CONFIRMED ×3] (captured 2026-07-10): Never close a cross-platform conflict on a single research pass — an independent adversarial verifier routinely overturns a premature 'disproven'.** When working the disagreement backlog (VITAL / PARENTAGE / IDENTITY / CONFLATION), every research verdict must go to a second agent whose job is to *refute* it: re-fetch the platform profile AND an independent down-line child (not the first analyst's cited child), and default to 'held' on any doubt. The verifier repeatedly flips "same person, spurious flag" back to held when the stored id resolves to a wrong-century same-name person, or when the date gap is a two-person-fusion fingerprint — both invisible to research that trusts the stored mapping. Run the verdict processor with a require-verify flag so a research-only verdict can never drain the backlog unverified. *(genealogy, dry-cross, kindred — the 2026-07 PARENTAGE + VITAL drain)*

**Rule [CONFIRMED ×3] (captured 2026-07-10): Split a large disagreement backlog into a mechanical tier (batch-close by rule) and a genuine tier (agent-research), and never cross the boundary.** Roughly half of cross-platform disagreements are same-person write-variants, not real conflicts: place granularity (one string ⊆ the other), county-split / town-match (Cordele in Crisp vs Dooly — Crisp County was formed from Dooly), spelling/typo (≥0.85 token similarity), ≤4-year date-estimate noise, same-state-plus-confirmed-id precision, and the platform combined-parent-string scrape. These close as *disproven*, journal-only and reversible, ONLY where identity is not itself in question (confirmed cross-platform id, no open conflation). Never batch-close the graft-prone calls — a name-string heuristic cannot distinguish a competing same-role parent from a legitimate co-parent, and one wrong auto-close buries a real problem. Reserve agent-research for the genuine remainder, and within it prioritise shallow/recent (high real-re-point yield) over the deep medieval-nobility tail (fabricated-graft / name-variant noise that no surviving record resolves). *(genealogy, dry-cross, kindred 2026-07)*

**Rule [CONFIRMED ×3] (captured 2026-07-10): A large VITAL date/place gap is usually an identity problem in disguise — research it, don't dismiss it.** A ~20-year death-date disagreement on a "confirmed" person typically means the linked profile is a different same-name individual or two people are fused, not that one date was mistyped. Chasing "which value is right" on shallow, well-sourced people is one of the cheapest fusion / wrong-id detectors available: the 2026-07 shallow-VITAL loop surfaced 8 two-person fusions and 4 wrong-id mappings across 74 date conflicts that no producer had flagged. The benign opposite is the stale snapshot — the captured platform value has since been corrected upstream, so a live re-fetch shows the platforms now agree; disprove those. Also: the *held* pile is the product, not the disproven count — a held verdict carries the recommended fix and the next record to pull, so keep it as a curated worklist. *(genealogy, dry-cross, kindred 2026-07)*
...
**Rule [CONFIRMED ×2] (captured 2026-04-15): Generation-shifted siblings are easy to add wrong.** Siblings should be the same generation as the direct-line sibling (Gen N), not the parents' generation (Gen N-1). Copy generation from the sibling, not the parent record. *(dry-cross, kindred)* **Batch-detect + fix (2026-07-10):** collateral sibling-placeholder nodes stored at the parent's generation instead of parent−1 are the dominant source of "convergence" generation warnings. A generation-offset fixer that canonicalizes only root-reachable ancestors (shortest path up to the root person) will miss them, because a collateral sibling has no up-chain path to the root — so fix them directly to `parent_gen − 1`, aligning with the already-correct sibling peers. In one dry-cross pass this cleared 148 of 162 warnings (74 sibling nodes × 2 parent edges each), journal/tree-structure only.

**Rule [CONFIRMED ×2] (captured 2026-04-15): Multi-lineage convergence creates intentional generation-diff artifacts.** When the same person appears in two lineages at different generation depths, the generation mismatch in validation is expected, not a bug. Document as convergence in triage notes. *(dry-cross, genealogy)* **These are graph-theoretically irreducible (2026-07-10):** once the sibling off-by-ones above are fixed, the residual convergence warnings cannot be resolved by editing generation numbers at all — a single integer generation per node cannot satisfy `parent = child + 1` on *every* edge when an ancestor is reached through descent paths of different lengths (pedigree collapse). Proof by attempt: recomputing the stored generations to the canonical BFS-minimum can *increase* the warning count, and a coordinated full-branch renumber introduces hard reference errors elsewhere. Leave them documented as expected; do not churn generations chasing zero. The validator should classify a convergence-ancestor's diff≠1 edges as WARN, not ERROR, and the gate should key on errors only.

**Rule [CONFIRMED ×3] (captured 2026-04-15): Don't commit regeneratable reports.** Raw script outputs (JSON reports, harvest results, pipeline intermediates like `fs_leads_batch_*`, `judge_batch_*`, `conflicts_*`, brick-wall stages) do not belong in git. They inflate commit sizes past push timeouts. *Prefer one blanket `data/reports/` line over chasing per-family timestamped patterns:* gitignore never untracks already-tracked files, so a blanket ignore hides every untracked regeneratable output while the curated decision-record `.md` files stay tracked, and a `docs/REPORTS_ARCHIVE_INDEX.md` catalogs which `.json`/`.md` are deliberately retained. Keep the sister repos at parity — a family that clutters `git status` in one tree is regeneratable scratch in all of them. Runtime state files (`.fs_harvest_progress.json`, `research_loop_state.json`) are the exception: they are tracked on purpose, so commit their updates rather than reverting. *(dry-cross, genealogy, kindred — kindred brought to blanket-ignore parity 2026-07-10)*
